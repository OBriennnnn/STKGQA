...
class STComplEx(nn.Module):
    """时空知识图谱嵌入模型 (STComplEx)"""

    def __init__(self, sizes, rank, init_size=1e-3):
        super().__init__()
        self.sizes = sizes  # (实体数, 关系数, 时间数, 位置数)
        self.rank = rank

        # 实体、关系、时间、位置嵌入层
        self.embeddings = nn.ModuleList([
            nn.Embedding(sizes[0], 2 * rank),  # 实体
            nn.Embedding(sizes[1], 2 * rank),  # 关系
            nn.Embedding(sizes[2], 2 * rank),  # 时间
            nn.Embedding(sizes[3], 2 * rank)  # 位置
        ])
    ...
    def forward(self, x):
        """前向传播"""
        lhs = self.embeddings[0](x[:, 0])  # 主体实体
        rel = self.embeddings[1](x[:, 1])  # 关系
        rhs = self.embeddings[0](x[:, 2])  # 客体实体
        time = self.embeddings[2](x[:, 3])  # 时间
        loc = self.embeddings[3](x[:, 4])  # 位置
        ...
        # 时空融合 (论文公式2)
        rel = (rel[0] * time[0] * loc[0], rel[1] * time[0] * loc[0])

        # 计算得分
        return torch.sum(
            (lhs[0] * rel[0] - lhs[1] * rel[1]) * rhs[0] +
            (lhs[1] * rel[0] + lhs[0] * rel[1]) * rhs[1],
            dim=1, keepdim=True
        )